	...
	private void initialize() {
		...
		FSalt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e){
				  salt = new Salt();
	                pan.AddSalt(salt);
	                FSalt.setEnabled(false);
			} });
		
		JRadioButton FOpen = new JRadioButton("\u041E\u0442\u043A\u0440\u044B\u0442");
		...
		JRadioButton FClose = new JRadioButton("\u0417\u0430\u043A\u0440\u044B\u0442");
		...
		JButton FWash = new JButton("\u041C\u044B\u0442\u044C \u044F\u0439\u0446\u0430");
		FWash.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				if (eggs == null)
	            {
					JOptionPane.showMessageDialog(frame, "Яиц то нет, что мыть? Допустим, что руки уже чистые :)");
	      ...
	      //          MessageBox.Show("Кран закрыт, как мыть?", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	            ...
	            pan.Init(Integer.parseInt((spinner.getValue()).toString()));
	            ...
	            FClose.setSelected(true);
	            JOptionPane.showMessageDialog(frame,"Яйца помыли, можно разбивать");
		...
		JButton FHit = new JButton("\u0420\u0430\u0437\u0431\u0438\u0442\u044C \u044F\u0439\u0446\u0430");
		FHit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
	         ...
	         //       MessageBox.Show("Яиц то нет, что разбивать собрались?", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	            ...
	            for (int i = 0; i < eggs.length; ++i)
	            {
	                if (eggs[i].GetDirty())
	                {
	                	 JOptionPane.showMessageDialog(frame,"Они же грязные! Фу!");
	     ...
	     //           MessageBox.Show("Без масла пригорит", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	            ...
	            JOptionPane.showMessageDialog(frame,"Яйца разбили на сковороду. Можно готовить");
		...
		JButton FCook = new JButton("\u0413\u043E\u0442\u043E\u0432\u0438\u0442\u044C ");
		FCook.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (eggs == null)
	            {
					 JOptionPane.showMessageDialog(frame,"Яиц то нет, воздух будем готовить?");
	  //              MessageBox.Show("Яиц то нет, воздух будем готовить?", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	                return;
	            }
	  ...
	  //              MessageBox.Show("Яиц то нет, воздух будем готовить?", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	            ...
	            for (int i = 0; i < eggs.length; ++i)
	            {
	                ...
	                {
	                	JOptionPane.showMessageDialog(frame,"Яичница со скорлупой. Любопытно, но нет");
	        //            MessageBox.Show("Яичница со скорлупой. Любопытно, но нет", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	                    return;
	                }
	            }
	            ...
	            if (!stove.GetState())
	            {
	            	JOptionPane.showMessageDialog(frame,"Ну это мы так долго будем готовить! Плиту включите.");
		...
		JButton FGet = new JButton("\u041F\u043E\u043B\u043E\u0436\u0438\u0442\u044C \u044F\u0438\u0447\u043D\u0438\u0446\u0443 \u043D\u0430 \u0442\u0430\u0440\u0435\u043B\u043A\u0443");
		FGet.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
	            ...
	            if (eggs == null)
	            {
	            	JOptionPane.showMessageDialog(frame,"Ага, конечно. Ну можете тарелку погрызть");
	         //       MessageBox.Show("Ага, конечно. Ну можете тарелку погрызть", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	                return;
	            }
	       ...
	       //         MessageBox.Show("Ага, конечно. Ну можете тарелку погрызть", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	         ...
	         //           MessageBox.Show("Яичница со скорлупой. Любопытно, но нет", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	            ...
	            {
	                eggs = pan.GetEggs();
	                JOptionPane.showMessageDialog(frame,"Мы сделали это! Приятного аппетита!");
	        //        MessageBox.Show("Мы сделали это! Приятного аппетита!", "Кухня", MessageBoxButtons.OK, MessageBoxIcon.Information);
	            }
	            else
	            {
	            	 JOptionPane.showMessageDialog(frame,"Ну сырые яйца - это, наверное, тоже съедобно. Но может все-таки приготовим их сначала");
	       //         MessageBox.Show("Ну сырые яйца - это, наверное, тоже съедобно. Но может все-таки приготовим их сначала", "Ошибка логики", MessageBoxButtons.OK, MessageBoxIcon.Error);
	                return;
	            }
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			//	int y=Integer.parseInt((spinner.getValue()).toString());
